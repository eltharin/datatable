...
$(document).on( 'preInit.dt', function (e, oSettings) 
{
	...
	console.log("preinit");
	console.log(oSettings.oSavedState);
});
...
{
	if(oSettings.oSavedState != null)
	{
		console.log(oSettings.oSavedState);
		$( oSettings.oSavedState.columns ).each(function( index ) {
			var search = $( this ).get(0).search.search;
			console.log(index);
			console.log(jQuery.type(search));
			if(jQuery.type(search) == "array")
			{

			}
			else if(jQuery.type(search) == "string" && search != "")
			{
				oSettings.zfilter.cols[index] = "toto";
