(function(window, document, $) {
    $.fn.dataTable.ext.filter = function ( settings ) {
        ...
        settings.aoInitComplete.push( {
            ...
            "fn": function (settings) {
                ...
                for (var i = 0; i < thLen; i++)
                {
                    ...
                    {
                        //settings.zfilter.cols[i] = settings.oSavedState.columns[i].zfilter ?? "";
                    ...
                    else
                    {
                        ...
                        var value = "";
                        if(settings.oLoadedState.columns[i].zfilter != undefined && settings.oLoadedState.columns[i].zfilter != "")
                        {
                            value = settings.oLoadedState.columns[i].zfilter;
                            settings.zfilter.cols[i] = value;
                        }
                        else if(settings.oLoadedState.columns[i].search.search != undefined)
                        {
                            value = settings.oLoadedState.columns[i].search.search;
                        }
                        else if(th[i].data('search'))
                        {
                            value = th[i].data('search');
                        }

                        var input = $('<input class="'+oClasse.iFilter+'" type="search" value="' + value + '">');
                        var timer;

                        input.appendTo(myth).on( 'change keyup search clear', function () {
                            var that = $(this);
        ...
        settings.aoStateSaveParams.push( {
            "sName" : "filter_widget",
            "fn": function (settings, data) {
                //console.log('saveParams');
                settings.zfilter.cols.forEach(function (a,b) {
                    settings.oSavedState.columns[b].zfilter = a;
                });
            }
        });

        settings.aoStateLoadParams.push( {
            "sName" : "filter_widget",
            "fn": function (settings, state) {
